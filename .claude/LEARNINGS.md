...
**Root cause:** D-10's "trace renders literals" rule sounded like locality (no system_params import) but actually broke locality — the gate value lives in two places (engine resolution + render literal) and they drift the moment any per-market override exists. The renderer was re-deriving an answer the engine had already computed instead of reading what the engine actually used.
**Fix:** New `signal_engine.resolve_vote_params(settings)` returns the resolved 4-tuple (adx_gate, momentum_threshold, momentum_votes_required, direction_mode). `get_signal` calls it (single resolution path). `daily_run.py` persists the result as `sig['vote_params']` next to `indicator_scalars` on every signal-row write. `_render_trace_panels` reads `vote_params` from sig_dict; `_render_trace_vote(indicator_scalars, signal, vote_params)` uses it for the gate badge/text AND the prelim Vote line. Defensive fallback to 25.0 / 0.02 keeps old state.json rows rendering until next daily run overwrites them.
**Prevention:** When a renderer's job is to audit a decision the engine already made, it MUST read the inputs+outputs the engine recorded, not re-derive them from defaults. Grep rule: any literal numeric threshold in `dashboard_legacy/` or `dashboard_renderer/` that matches a constant in `system_params.py` is a drift smell — `grep -nE "= 25\.0|= 0\.02|= 1\.5" dashboard_legacy/ dashboard_renderer/`. Treat the persisted signal row as the audit record; new params added to `get_signal` must also be added to `resolve_vote_params` AND surfaced in the trace dict, or the trace panel will silently lie the next time someone overrides the default.
**Date:** 2026-05-10
