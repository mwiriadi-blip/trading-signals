...
<p class="manual-secret" id="manual-secret-text">{manual_secret}</p>
<button type="button" class="copy-secret-btn"
  onclick="navigator.clipboard.writeText(document.getElementById('manual-secret-text').textContent.replace(/\s/g,'')).then(function(){{this.textContent='Copied!';setTimeout(function(){{document.querySelector('.copy-secret-btn').textContent='Copy'}},2000)}}.bind(this))">Copy</button>
</div>
