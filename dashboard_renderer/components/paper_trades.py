...
Phase 32 Plan 02: absorbs 6 unique functions from dashboard_legacy/paper_trades_section.py.
Existing public render_paper_trades_region (no underscore) preserved as canonical entrypoint
and rewired to canonical dashboard_renderer.* imports.
'''
...
from dashboard_renderer.formatters import _fmt_em_dash
from dashboard_renderer.stats import compute_aggregate_stats
...
def render_paper_trades_region(state: dict) -> str:
  ...
  PUBLIC entrypoint — no underscore prefix. Canonical name locked per
  32-02-PLAN.md §CANONICAL NAMING LOCK. Downstream plan 32-04 Task 1+2
  import this exact name.
  '''
  paper_trades = state.get('paper_trades', []) or []
  signals = state.get('signals', {})
  stats = compute_aggregate_stats(paper_trades, signals)
  ...
  stats_html = _render_paper_trades_stats(stats) if (closed_paper + closed_live) >= 1 else ''
