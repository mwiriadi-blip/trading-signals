...
|----------|----------|-----------|-------------------|-------------|--------|
| CHORE-01 SC-1 | Full chain fetch→email with section + key-value assertions; mocking only at `requests.get` + `_post_to_resend`; both instruments active in one fixture; uses `notifier/sample_state_with_change.json` as state seed (drift+positions present) | integration | `.venv/bin/pytest tests/test_integration_f1.py::test_full_chain_fetch_to_email -x -q` | ❌ W0 | ⬜ pending |
| CHORE-01 SC-2 | Meta-test demonstrates F1 red-lights when `signal_engine.get_signal` is monkey-patched (rename simulation); F1 passes WITHOUT patch (sanity check) | integration | `.venv/bin/pytest tests/test_integration_f1.py::test_f1_catches_planted_regression -x -q` | ❌ W0 | ⬜ pending |
...
| SC-4 | STATE.md `## Completed Items` exists with 3 migrated items (Phase 06 HUMAN-UAT, Phase 05 dashboard verification_gap, Phase 06 email verification_gap) and verification dates | manual | `grep -c "^## Completed Items" .planning/STATE.md` returns 1 AND `grep -A 10 "## Completed Items" .planning/STATE.md \| grep -c "verified"` returns ≥ 3 | ❌ W0 | ⬜ pending |
