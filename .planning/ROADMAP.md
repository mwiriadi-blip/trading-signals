...
**Plans:** 4 plans
- [x] 06-01-PLAN.md — Wave 0 scaffold: notifier.py stub + palette retrofit (dashboard→system_params) + AST blocklist extension (FORBIDDEN_MODULES_NOTIFIER) + tests/test_notifier.py 6-class skeleton + 3 JSON fixtures + 3 placeholder HTML goldens + regenerator script + .env.example + .gitignore last_email.html
- [x] 06-02-PLAN.md — Wave 1 render + format: compose_email_subject (D-04) + compose_email_body (D-07/D-10 7 sections + D-11 ACTION REQUIRED red-border block) + _detect_signal_changes (D-06) + 7 email formatters + TestComposeSubject/TestDetectSignalChanges/TestFormatters/TestComposeBody (~50 tests) + XSS escape hardening (NOTF-09)
- [x] 06-03-PLAN.md — Wave 2 PHASE GATE dispatch + integration: _post_to_resend retry loop (429 retryable per RESEARCH §1; 400/401/403/422 fail-fast) + send_daily_email never-crash + _atomic_write_html + main.py _send_email_never_crash + run_daily_check 4-tuple return + TestSendDispatch/TestResendPost/TestGoldenEmail + TestCLI force-email/test-flag integration + TestEmailNeverCrash + 3 committed byte-stable golden HTML snapshots (regenerator double-run idempotent)
- [ ] 06-04-PLAN.md — Wave 3 gap_closure: WR-01 run_daily_check docstring reworded to reflect exception-propagation contract + None-guard framed as defense-in-depth; WR-02 compose_email_subject emits first run label when date_iso empty + golden_empty_subject.txt regenerated + new TestComposeSubject pinning test (cosmetic + docs only — NOTF-01..09 behaviour unchanged, regenerator double-run idempotency preserved)
**UI hint**: no
...
**Plans:** 3/3 planned
- [ ] 08-01-PLAN.md — Wave 1 foundation: system_params contract-tier dicts, state_manager MIGRATIONS[2] v2 schema backfill + _resolved_contracts materialisation in load_state + underscore-prefix exclusion in save_state + clear_warnings helper + TestMigrateV2Backfill/TestSaveStateExcludesUnderscoreKeys/TestLoadStateResolvesContracts/TestClearWarnings (CONF-01/CONF-02/ERR-03)
- [ ] 08-02-PLAN.md — Wave 2 notifier: SendStatus NamedTuple + two-tier banner (_render_header_email D-01/D-03 age filter) + compose_email_subject `[!]` prefix + always-write last_email.html + send_crash_email text/plain dispatcher + _post_to_resend text_body support + TestHeaderBanner/TestSubjectCriticalPrefix/TestSendDispatchStatusTuple/TestLastEmailAlwaysWritten/TestCrashEmail/TestPostToResendContentType (NOTF-10/ERR-02/ERR-03/ERR-04/ERR-05)
