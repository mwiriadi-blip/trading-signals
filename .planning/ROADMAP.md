...
- [x] 27-05-magic-cost-helper-and-fallback-email-PLAN.md — Wave 1C (depends on 27-01 for Decimal): pnl_engine.entry_side_cost(rt_cost) -> Decimal helper replaces 4 cost/2 literals (notifier:494, main:1324, main:1428 close-half, main:1535); _EMAIL_TO_FALLBACK constant deleted; SIGNALS_EMAIL_TO required across send_daily_email + send_crash_email + send_stop_alert_email; missing → log ERROR + state-health warning marker + fail-soft skip. (completed 2026-05-08; 17 behavioral tests; 1880/1880 full suite green; SUMMARY at .planning/phases/27-…/27-05-SUMMARY.md)
...
- [x] 27-09-signal-shape-unification-PLAN.md — Wave 2 (depends on 27-01 + 27-07): drop bare-int signal back-compat; schema bump + migrator; 38 test sites refactored; renderer isinstance(int) branch deleted.
- [x] 27-10-warnings-fifo-rundate-lookahead-tests-PLAN.md — Wave 2: WARNINGS_FIFO_MAX_LEN=50 bound + eviction test; daily-run-date INFO log assertion; backtest look-ahead-bias proof.
...
- [x] 27-15-notifier-py-fossil-cleanup-PLAN.md — Wave 4 (depends on 27-12 + 27-14): comment-only sweep of stale `notifier.py` references in production source describing CURRENT architecture (system_params.py:264,336; crash_boundary.py:39; web/app.py:4; web/middleware/auth.py:33; tests/test_setup_https_doc.py:7,281). 5 files / 7 line-edits. Bucket B fossils (CR-01 fix breadcrumbs, Plan 27-12 provenance docstrings inside notifier/*.py, `tests/test_notifier.py` path constants) preserved. (completed 2026-05-08; 2028/2028 full suite green; SUMMARY at .planning/phases/27-…/27-15-SUMMARY.md)
- [x] 27-16-deployment-doc-sweep-do-primary-PLAN.md — Wave 4 (depends on 27-15): retire stale GHA-primary / Replit-alternative prose; document DigitalOcean droplet + systemd as primary. Full rewrite of docs/DEPLOY.md (152 lines, droplet-primary with SETUP-DROPLET.md cross-references); README.md (drop stale GHA badge + ${{GITHUB_REPOSITORY}} placeholder paragraph; rewrite Deployment section); scheduler_driver.py:122 + state_actions.py:21 comments; tests/test_scheduler.py::TestDeployDocs rewritten (4 dropped, 6 added; TestGHAWorkflow:12 preserved as rollback insurance per Phase 10 INFRA-03); .planning/PROJECT.md:85,113 stale-doc caveats dropped. Bucket B fossils (v1.0 phase 07 archive, SPEC.md historical brief, daily.yml.disabled, TestGHAWorkflow) preserved. (completed 2026-05-10; 2013 passed / 17 pre-existing test_main.py failures unchanged from HEAD; SUMMARY at .planning/phases/27-…/27-16-SUMMARY.md)
