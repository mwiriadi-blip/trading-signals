# v1.1 Milestone Archive — Trading Signals

**Status:** SHIPPED 2026-04-30
**Phases:** 10, 11, 12, 13, 14, 15, 16, 16.1 (8 phases, 38 plans, all complete)
**Requirements:** 40/40 mapped, 100% coverage — see [v1.1-REQUIREMENTS.md](./v1.1-REQUIREMENTS.md)
**Timeline:** 2026-04-25 → 2026-04-30 (6 days)
**Tag:** `v1.1`
**Tests at close:** 1319 passing / 12 deferred (nginx config drift + ruff binary missing in test env, all logged in `.planning/phases/16.1-phone-friendly-auth-ux-for-dashboard-access/deferred-items.md`)
**Git range:** `b1f9b8f..cf20a78` (179 commits, 166 files, +57,623 / −264 LOC)

## Headline Accomplishments

1. **Hosted dashboard live** — `https://signals.mwiriadi.me/` publicly accessible with HTTPS + HSTS + Let's Encrypt cert + auth gate
2. **Trade journal mutations** — operator records open/close/modify positions via HTMX forms; `state_manager.save_state()` sole-writer invariant preserved
3. **Phone-friendly auth UX** (Phase 16.1) — cookie session + TOTP enroll/verify, trusted-device opt-in (30-day signed cookie), magic-link 2FA recovery
4. **Drift sentinel pipeline** — D-12 lockstep parity between email banner and dashboard banner row, `[!]` subject prefix on critical events
5. **F1 full-chain integration test** — boundary-only mocks, sabotage-test verified (planted regression red-lights it)
6. **Operator UAT closure** — UAT-16-A/B/C all verified through real-world deployment + drift email observation in production Gmail mobile

## Notable mid-milestone fix (quick task `260429-sdp`)

Silent regression in `main.py::_run_daily_check_caught` (commit `3279c312`, 2026-04-23) discarded the 4-tuple from `run_daily_check(args)` and stopped the production droplet daemon from sending daily emails for ~7 days. Diagnosed via journalctl pattern matching, fixed in commit `879730d` with 4 regression tests + inverted Phase-4 fossil test. Captured as global learning in `~/.claude/LEARNINGS.md`.

## Deferred to v1.2

- 9 operator-driven UAT scenarios across Phases 13/14/16.1 (require live droplet + browser/phone hands-on; system shipping cleanly through them)
- Phase 13 13-VERIFICATION.md `human_needed` (same droplet/curl constraint)
- 1 quick task from v1.0 (`260421-723-add-oracle-hash-comment-test-compute-ind`) — out of v1.1 and v1.2 scope
- See [STATE.md §Deferred Items](../STATE.md#deferred-items) for full disposition

---

## Original Milestone Definition (preserved for audit trail)
